...
def get_weather_type(temperature):
    for weather_type in weatherforecast_types_list:
        if (weather_type.temperature_min is None or temperature >= weather_type.temperature_min) and \
           (weather_type.temperature_max is None or temperature < weather_type.temperature_max):
